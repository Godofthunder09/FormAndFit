<?php
// Dummy data setup for a Hoodie product

// Dummy Hoodie ID (used for links/tracking only)
$hoodie_id = "1-Hoo"; // Changed ID for a Hoodie

// Dummy Product Data
$hoodie = [
    'id' => $hoodie_id,
    'name' => 'Lost In Thought - Serpent/Vine Graphic Tee', // Updated Name
    'price' => '32', // Updated Price
    'description' => 'Dive deep into the darkness with the "Lost In Thought" graphic. Featuring a complex, interwoven central motif that resembles tangled serpents, roots, or veins, this design captures a feeling of introspection and being consumed by thought. The distressed, grunge-style overlay on the repeated "LOST IN THOUGHT" text adds a powerful, moody atmosphere. Details include "EST. MMXXIX24" and the subtle motto "wander alone," perfect for those who embrace solitude and deep reflection. The dark teal/slate and muted earth tones give it a vintage, underground feel.', // Updated Description
    // ⬇️ FIX 1: COMMA ADDED
    'alt_text' => "Distressed grunge graphic featuring repeated \"LOST IN THOUGHT\" text and a dark, interwoven tangle of abstract shapes like snakes or vines in the center. Includes \"EST. MMXXIX24\" and \"wander alone.\" Dark teal, grey, and brown color palette.",
    // ⬇️ FIX 1: COMMA ADDED
    'buy_link' => 'https://www.teepublic.com/hoodie/83038014-lost-in-thought-t-shirt-hoodie', // Dummy buy link
];

// 🔴 CRITICAL FIX: The relative paths below will likely break unless you replace them
// with full URLs or correct relative paths to your public images.
$DUMMY_IMAGE_LINKS = [
    // === PASTE YOUR ACTUAL HOODIE IMAGE LINKS HERE ===
    'admin/image_uploads/hoodies_1-Hoo_1.png', // <-- Replace this with your first link
    'admin/image_uploads/hoodies_1-Hoo_2.png', // <-- Replace this with your second link
    'admin/image_uploads/hoodies_1-Hoo_3.png', // <-- Replace this with your third link
    'admin/image_uploads/hoodies_1-Hoo_4.png'  // <-- Replace this with your fourth link
    // ===================================
];
...
// Define a variable to use throughout the page, like $tshirt was used before
$product = $hoodie; // $product is the new variable name
...
<head>
    ...
    <title><?php echo htmlspecialchars($product['name']); ?> - FormAndFit</title>
...
<body class="bg-gray-50">
    ...
    <div class="max-w-6xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div>
                <h1 class="text-4xl font-extrabold text-gray-900 leading-tight"><?php echo htmlspecialchars($product['name']); ?></h1>
                ...
                <div class="mt-6 p-4 bg-gray-100 rounded-lg shadow-inner">
                    ...
                    <p class="text-gray-700 mb-4"><?php echo nl2br(htmlspecialchars($product['description'])); ?></p>
                    
                    <h3 class="text-lg font-semibold text-gray-800 mb-1">Specs & Quality</h3>
                    
                    <p class="text-gray-700 whitespace-pre-line"><?php echo nl2br(htmlspecialchars($product['alt_text'] ?? 'No detailed specs available.')); ?></p>
                </div>
                <hr class="my-8">
                ...
                <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                    ...
                    <?php if (!empty($product['buy_link'])): ?>
                        <a href="<?php echo htmlspecialchars($product['buy_link']); ?>" target="_blank"
                            ...
                            data-buy-link="<?php echo htmlspecialchars($product['buy_link']); ?>">
                            🛒 Buy Now
                        </a>
                    <?php else: ?>
                        <button disabled
        ...
        <div class="bg-white rounded-lg shadow-2xl max-w-sm w-full p-6 transform transition-all">
            ...
            <div class="flex mb-6">
                <input id="product-share-link" type="text" readonly
                    value="<?php echo htmlspecialchars($product['buy_link'] ?? ''); ?>" 
            ...
            <div class="flex space-x-4 justify-center">
                <a href="https://twitter.com/intent/tweet?url=<?php echo urlencode($product['buy_link'] ?? ''); ?>&text=Check out this awesome <?php echo urlencode($product['name']); ?> from FormAndFit!" target="_blank" class="text-blue-400 hover:text-blue-500" aria-label="Share on Twitter">
                    <i data-feather="twitter" class="w-7 h-7"></i>
                </a>
                <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode($product['buy_link'] ?? ''); ?>" target="_blank" class="text-blue-600 hover:text-blue-700" aria-label="Share on Facebook">
                    <i data-feather="facebook" class="w-7 h-7"></i>
                </a>
                <a href="mailto:?subject=Cool T-Shirt from FormAndFit!&body=I found this awesome t-shirt, the <?php echo htmlspecialchars($product['name']); ?>. Check it out here: <?php echo urlencode($product['buy_link'] ?? ''); ?>" target="_blank" class="text-gray-700 hover:text-gray-900" aria-label="Share via Email">
...
<script>
    ...
    // (This section correctly uses data attributes from the HTML, which we just fixed)
    ...
    function sendTrackingRequest(clickType) {
        ...
        console.log(`[Tracking] Attempting to send data: ID=${productId}, Name=${productName}, Type=${clickType}, Category=${productCategory}`);
